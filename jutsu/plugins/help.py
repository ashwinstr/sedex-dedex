...
from jutsu.utils import get_file_id, get_response
from jutsu.utils import parse_buttons as pb
from jutsu.utils import rand_key
...
if sedex.has_bot:
    ...
    @sedex.bot.on_callback_query(filters.regex(pattern=r"back\((.+)\)"))
    @check_owner
    ...
    @sedex.bot.on_callback_query(filters.regex(pattern=r"enter\((.+)\)"))
    @check_owner
    ...
    @sedex.bot.on_callback_query(filters.regex(pattern=r"^mm$"))
    @check_owner
    ...
    @sedex.bot.on_callback_query(filters.regex(pattern=r"^chgclnt$"))
    @check_owner
    ...
    @sedex.bot.on_callback_query(filters.regex(pattern=r"refresh\((.+)\)"))
    @check_owner
    ...
    def category_data(cur_pos: str):
        pos_list = cur_pos.split("|")
        plugins = sedex.manager.get_all_plugins()[pos_list[1]]
        text = (
    ...
    def plugin_data(cur_pos: str, p_num: int = 0):
        pos_list = cur_pos.split("|")
        plg = sedex.manager.plugins[pos_list[2]]
