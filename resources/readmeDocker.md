...
- Setup configs
    - Download the sample config file <br>
        - ```mkdir sedex-x && cd sedex-x```
        - ```wget https://raw.githubusercontent.com/code-rgb/sedex-x/alpha/config.env.sample -O config.env```
        - ```vim config.env```
    - Download the yml file for docker-compose
        - ```wget https://raw.githubusercontent.com/code-rgb/sedex-x/alpha/resources/docker-compose.yml```
